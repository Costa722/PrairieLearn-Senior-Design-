...
mkdir "$HOME/pl\_ag\_jobs"

Running Docker with the extended features

Now, we can run PrairieLearn with additional options to allow the external grading or workspaces features. For example, if your course directory is in $HOME/pl-tam212 and the jobs directory created above is in $HOME/pl\_ag\_jobs, the new command is as follows:

docker run -it --rm -p 3000:3000 \

- v "$HOME/pl-tam212:/course" `# Replace the path with your course directory` \
- v "$HOME/pl\_ag\_jobs:/jobs" `# Map jobs directory into /jobs` \
- e HOST\_JOBS\_DIR="$HOME/pl\_ag\_jobs" \
